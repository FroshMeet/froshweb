...
import { Instagram, Search, ExternalLink, Smartphone, Users, MessageSquare, Star, Heart, Globe, Zap, QrCode, ArrowRight, Play } from "lucide-react";
...
export default function SchoolCampusHub() {
    ...
    const fetchStudentProfiles = async () => {
      ...
      try {
        ...
        const profiles: StudentProfile[] = (data || []).map(profile => ({
          ...
          name: profile.name,
          major: 'Student',
          // Default since major not in instagram_profiles
  ...
  const searchStudents = useCallback(async (query: string) => {
    ...
    // Simulate API delay
    setTimeout(() => {
      const filtered = studentProfiles.filter(profile => profile.name.toLowerCase().includes(query.toLowerCase()) || profile.major.toLowerCase().includes(query.toLowerCase()) || profile.instagram_handle?.toLowerCase().includes(query.toLowerCase()));
  ...
  return <div className="min-h-screen bg-background">
      ...
      <section className="relative py-20 px-4 bg-gradient-to-br from-background via-muted/10 to-background overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative">
          ...
          {instagramHandle && <div className="mb-8">
              <Button variant="outline" onClick={() => handleViewInstagram(instagramHandle)} className="group border-primary/30 text-primary hover:bg-primary/10 px-6 py-3 rounded-full font-bold text-lg transition-all duration-300 hover:scale-105">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
            <Button onClick={handleOpenApp} size="lg" className="bg-primary hover:bg-primary/90 text-primary-foreground px-8 py-4 rounded-full font-bold text-lg shadow-lg shadow-primary/25 transition-all duration-300 hover:scale-105 neon-glow">
              <Smartphone className="h-5 w-5 mr-2" />
              Open in App
            </Button>
            <Button onClick={handleTextMeApp} variant="outline" size="lg" className="border-primary/30 text-primary hover:bg-primary/10 px-8 py-4 rounded-full font-bold text-lg transition-all duration-300 hover:scale-105">
      ...
      <section className="py-16 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center">
            <Button onClick={() => instagramHandle && handleViewInstagram(instagramHandle)} variant="outline" className="border-primary/30 text-primary hover:bg-primary/10 px-6 py-3 rounded-full font-bold">
              <Instagram className="h-5 w-5 mr-2" />
              Follow on Instagram
            </Button>
          </div>
        ...
        <div className="max-w-4xl mx-auto text-center">
          <Card className="bg-gradient-to-br from-primary/20 via-primary/10 to-primary/5 border-primary/20 p-8 rounded-3xl">
            <CardContent className="p-0">
              ...
              <Button onClick={handleGetFeatured} size="lg" className="bg-primary hover:bg-primary/90 text-primary-foreground px-8 py-4 rounded-full font-bold text-lg shadow-lg shadow-primary/25 transition-all duration-300 hover:scale-105 neon-glow">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="max-w-2xl mx-auto mb-12">
            <div className="relative">
              ...
              <Input placeholder="Search students..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-12 pr-4 py-4 text-lg rounded-full border-border/40 bg-muted/30 focus:bg-background transition-colors" />
            </div>
          </div>

          {/* Search Results */}
          {searchQuery && <div className="mb-12">
              {isSearching ? <div className="text-center py-8">
                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary mx-auto mb-4"></div>
                  <p className="text-muted-foreground">Searching...</p>
                </div> : searchResults.length > 0 ? <>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                    {searchResults.map(profile => <Card key={profile.id} className="group overflow-hidden bg-card/50 border-border/40 hover:bg-card/70 transition-all duration-300 hover:scale-105">
                        <CardContent className="p-6">
                          ...
                          {profile.bio && <p className="text-sm text-muted-foreground mb-4 line-clamp-2">
                              {profile.bio}
                            </p>}
                          {profile.instagram_handle && <Button variant="outline" size="sm" onClick={() => handleViewInstagram(profile.instagram_handle!)} className="w-full border-primary/30 text-primary hover:bg-primary/10">
                  ...
                  <div className="text-center">
                    <Button onClick={handleOpenApp} className="bg-primary hover:bg-primary/90 text-primary-foreground px-6 py-3 rounded-full font-bold">
                      <MessageSquare className="h-5 w-5 mr-2" />
                      DM them in the app
                    </Button>
                  </div>
      ...
      {/* Sticky App Button */}
      {showStickyButton && <div className="fixed bottom-6 right-6 z-50 animate-fade-in">
          <Button onClick={handleOpenApp} size="lg" className="bg-primary hover:bg-primary/90 text-primary-foreground px-6 py-4 rounded-full font-bold shadow-2xl shadow-primary/30 transition-all duration-300 hover:scale-105 neon-glow">
            <Smartphone className="h-5 w-5 mr-2" />
            Open in App
          </Button>
        </div>}
      ...
      {/* Phone Modal */}
      {showPhoneModal && <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            ...
            <CardContent className="p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-3">
                  <Button onClick={handleSendSMSLink} className="flex-1 bg-primary hover:bg-primary/90" disabled={!phoneNumber}>
                    Send Link
                  </Button>
