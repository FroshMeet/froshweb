      ...
      <div className="bg-card border-b border-border p-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex space-x-6">
            ...
            <button className="text-froshmeet-blue font-semibold relative">
              Meet
              <div className="absolute -bottom-1 left-0 right-0 h-0.5 bg-froshmeet-blue rounded-full" />
            </button>
            <button className="text-muted-foreground hover:text-foreground transition-colors">
              Chat
            </button>
        ...
        <div className="flex items-center justify-between mt-4">
          <div className="flex space-x-1">
            <button
              onClick={() => setMeetMode && setMeetMode("everyone")}
              className={`px-4 py-2 rounded-2xl font-medium transition-all ${
                meetMode === "everyone"
                  ? "bg-froshmeet-blue text-white shadow-lg"
            ...
            <button
              onClick={() => setMeetMode && setMeetMode("roommates")}
              className={`px-4 py-2 rounded-2xl font-medium transition-all ${
                meetMode === "roommates"
                  ? "bg-froshmeet-blue text-white shadow-lg"
                  : "bg-muted text-muted-foreground hover:bg-muted/80"
          ...
          {meetMode === "everyone" && (
            <div className="flex items-center space-x-2">
              <button
                onClick={() => setScopeMode("school")}
                className={`px-3 py-1.5 rounded-xl text-sm font-medium transition-all ${
                  scopeMode === "school"
                    ? "bg-froshmeet-blue/20 text-froshmeet-blue"
          ...
          <Button
            ...
            className="border-froshmeet-blue/20 text-froshmeet-blue hover:bg-froshmeet-blue/10"
          >
      ...
      <div className="flex h-[calc(100%-120px)]">
        ...
        <div className="md:hidden w-full flex flex-col">
              ...
              <>
                <button
                  ...
                  className={`absolute left-4 top-1/2 -translate-y-1/2 bg-black/50 text-white p-2 rounded-full ${
                    currentPhotoIndex === 0 ? 'opacity-30' : 'hover:bg-black/70'
                  }`}
                >
                ...
                <button
                  ...
                  className={`absolute right-4 top-1/2 -translate-y-1/2 bg-black/50 text-white p-2 rounded-full ${
                    currentPhotoIndex === photos.length - 1 ? 'opacity-30' : 'hover:bg-black/70'
                  }`}
                >
          ...
          <div className="bg-card p-4 flex space-x-3">
            <Button 
              variant="outline" 
              className="flex-1 border-gray-300 text-gray-600 hover:bg-gray-50 hover:text-gray-700 h-12"
            ...
            <Button 
              className="flex-1 bg-froshmeet-blue hover:bg-froshmeet-blue-dark text-white h-12 hover:shadow-[var(--glow-blue)] transition-all duration-300"
            ...
            <Button 
              className="flex-1 bg-pink-500 hover:bg-pink-600 text-white h-12"
        ...
        <div className="hidden md:flex w-full">
          ...
          <div className="flex-1 relative">
            <img
              src={getUnsplashUrl(currentPhoto)}
              alt={currentProfile.name || "Profile"}
              className="w-full h-full object-cover rounded-2xl m-4"
            />
            ...
            {photos.length > 1 && (
              <>
                <button
                  onClick={prevPhoto}
                  disabled={currentPhotoIndex === 0}
                  className={`absolute left-8 top-1/2 -translate-y-1/2 bg-black/50 text-white p-3 rounded-full ${
                    currentPhotoIndex === 0 ? 'opacity-30' : 'hover:bg-black/70'
                  }`}
                >
                ...
                <button
                  onClick={nextPhoto}
                  disabled={currentPhotoIndex === photos.length - 1}
                  className={`absolute right-8 top-1/2 -translate-y-1/2 bg-black/50 text-white p-3 rounded-full ${
                    currentPhotoIndex === photos.length - 1 ? 'opacity-30' : 'hover:bg-black/70'
                  }`}
                >
            ...
            <Card className="h-full bg-card border border-border rounded-2xl shadow-lg">
              <CardContent className="p-6 h-full flex flex-col">
                <div className="space-y-6 flex-1">
                  ...
                  {/* Roommate Badge */}
                  {currentProfile.lookingFor?.includes("Roommate") && (
                    <Badge className="bg-purple-600/10 text-purple-600 border border-purple-600/20 w-fit">
                      <Users className="h-3 w-3 mr-1" />
                      Looking for Roommate
                    </Badge>
                  )}
                ...
                <div className="space-y-3 pt-6 border-t border-border">
                  <Button 
                    className="w-full bg-froshmeet-blue hover:bg-froshmeet-blue-dark text-white h-12 hover:shadow-[var(--glow-blue)] transition-all duration-300 rounded-xl"
                    onClick={handleMessage}
                  >
                    <MessageSquare className="h-5 w-5 mr-2" />
                    Message
                  ...
                  <div className="flex space-x-3">
                    <Button 
                      variant="outline" 
                      className="flex-1 border-gray-300 text-gray-600 hover:bg-gray-50 hover:text-gray-700 h-11 rounded-xl"
                      onClick={() => handleSwipe('skip')}
                    >
                    ...
                    <Button 
                      className="flex-1 bg-pink-500 hover:bg-pink-600 text-white h-11 rounded-xl"
                      onClick={() => handleSwipe('like')}
                    >
          ...
          <div className="space-y-6 py-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {["2025", "2026", "2027", "2028", "2029", "2030"].map((year) => (
                  <div key={year} className="flex items-center space-x-2">
                    <Checkbox 
                      id={year}
                      checked={filters.classYears.includes(year)}
            ...
            <div>
              <div className="flex items-center space-x-2">
                <Checkbox 
                  id="roommate"
                  checked={filters.lookingForRoommate}
                  onCheckedChange={(checked) => {
                    setFilters(prev => ({
                      ...prev,
            ...
            <div className="flex space-x-3">
              ...
              <Button 
                className="flex-1 bg-froshmeet-blue hover:bg-froshmeet-blue-dark text-white"
