...
-- Drop and recreate RLS policies for proper chat system
...
CREATE POLICY "Same school users can view profiles" 
ON public.profiles 
FOR SELECT 
USING (
  auth.uid() = user_id OR 
...
CREATE POLICY "Users can view conversations or group summaries" 
...
DROP POLICY IF EXISTS "Users can manage their own school memberships" ON public.school_conversation_members;
DROP POLICY IF EXISTS "Users can view their own school memberships" ON public.school_conversation_members;
...
CREATE POLICY "Users view group member counts" 
ON public.school_conversation_members 
FOR SELECT 
USING (
  user_id = auth.uid() OR 
